...
class RecurrsiveFeatureSelector:
    ...
    def start_trial(self, model, X, y, cv, k = None, n = 0, jump_start = False, base_com = None, n_select = 1):
        ...
        #-------------------------------------------------------Trial 1--------------------------------------------------------------
            ...
            for i in itertools.product(test_list[0:1],test_list[1:]):
                score = cross_val_score(model, X[(list(i))], y.values.ravel(), cv=3)
        ...
        if jump_start == True:
            no_trial = 1
            score = cross_val_score(model, X[base_com], y.values.ravel(), cv=3)
            ...
            for i in np.hstack([df1,df2]).tolist():
                    score = cross_val_score(model, X[i], y.values.ravel(), cv=3)

...
class RecurrsiveFeatureSelector:
    ...
    def trial(self, model, X, y, cv, task, scoring, max_round=None, chances_to_fail=None, jump_start=None, n_digit=4):
        ...
        if task == 'classification':
            plt.axvline(x = np.argmax(list(rfs.best_score.values())) + 1, color='green', linewidth=2, linestyle='--')
        if task == 'regression':
            plt.axvline(x = np.argmin(list(rfs.best_score.values())) + 1, color='green', linewidth=2, linestyle='--')
